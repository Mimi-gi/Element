...
### 乗り移りのビジュアライザ・カメラ追従の仕様について
#### 乗り移り
現状、乗り移りのビジュアライザが行われていないため、実装を行う。
まず、DefaultのDarkの子オブジェクトにEyeというものがあり、これが前述した目の役割を持つ。
乗り移りの際に行いたいこととしては、
①Eyeを乗り移り先の子に移し、相対位置をPossableのプロパティである「EyePos」にする(EyePosはまだ未実装なため、実装が必要)
②前のIPossableから後のIPossableにエフェクトを飛ばす。
##### エフェクト飛ばしについて、
とりあえず、
①乗り移り開始時にVFXをオン
②座標を前のIPossableのEyePosから後のIPossableのEyePosに移動させる(LitMotion?)
③後の座標に達したらVFXをオフ
↑はPossessionManagerで実装したほうが良い気がする。意見が欲しい

#### カメラ追従
現在、乗り移ったらカメラをそのIPossableに移動させているが、これを廃止
カメラ移動のキーのようなものをステージの各地に配置し、そこにプレイヤーが入ったら移動にするのが良さそう

↑について、カメラ・エフェクトの移動中は操作不可にする。

トリガーの配置の仕方
とりあえずコライダーを配置して、そこに入ったらカメラを移動という形にしたい。
カメラ移動は片方向のみ（戻り方向への自動移動はなし）。